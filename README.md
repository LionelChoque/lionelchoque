# Hi, I'm Lionel 👋

**Backend Developer** focused on building reliable, well-structured systems — from industrial data pipelines to web APIs and intelligent applications.

Based in **Mendoza, Argentina** · Open to remote opportunities

---

## What I work with

**Backend**

![Java](https://img.shields.io/badge/Java-ED8B00?style=flat&logo=openjdk&logoColor=white)
![Spring Boot](https://img.shields.io/badge/Spring_Boot-6DB33F?style=flat&logo=springboot&logoColor=white)
![Spring Security](https://img.shields.io/badge/Spring_Security-6DB33F?style=flat&logo=springsecurity&logoColor=white)
![Node.js](https://img.shields.io/badge/Node.js-339933?style=flat&logo=nodedotjs&logoColor=white)
![NestJS](https://img.shields.io/badge/NestJS-E0234E?style=flat&logo=nestjs&logoColor=white)

**Frontend**

![React](https://img.shields.io/badge/React-20232A?style=flat&logo=react&logoColor=61DAFB)
![Next.js](https://img.shields.io/badge/Next.js-000000?style=flat&logo=nextdotjs&logoColor=white)

**Data & Infrastructure**

![PostgreSQL](https://img.shields.io/badge/PostgreSQL-4169E1?style=flat&logo=postgresql&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-2496ED?style=flat&logo=docker&logoColor=white)
![Redis](https://img.shields.io/badge/Redis-DC382D?style=flat&logo=redis&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?style=flat&logo=git&logoColor=white)

**Industrial & AI/ML**

![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white)
![SCADA/IIoT](https://img.shields.io/badge/IIoT%20%2F%20SCADA-00599C?style=flat)
![Machine Learning](https://img.shields.io/badge/Machine_Learning-FF6F00?style=flat&logo=tensorflow&logoColor=white)

---

## Portfolio

Projects built to demonstrate real skills — each one deployable with a single `docker compose up`.

| # | Project | What it demonstrates | Stack | Status |
|---|---------|---------------------|-------|--------|
| 01 | [Java Core Playground](https://github.com/lionelchoque/java-core-playground) | OOP, Streams, Lambdas, Collections, JUnit 5 | Java 17, Maven | 🔨 Building |
| 02 | [Persistence Evolution](https://github.com/lionelchoque/java-persistence-journey) | JDBC → JPA → Hibernate — understanding the *why* behind ORMs | Java, JPA, Hibernate, PostgreSQL | 🔨 Building |
| 03 | [Data Integration Service](https://github.com/lionelchoque/data-integration-service) | Spring Boot · multiple datasources · external API consumption | Spring Boot, WebClient, PostgreSQL, MySQL | ⏳ Planned |
| 04 | [Fullstack — Spring + React](https://github.com/lionelchoque/fullstack-spring-react) | REST API + SPA + Nginx reverse proxy, fully dockerized | Spring Boot, React, Docker Compose | ⏳ Planned |
| 05 | [Spring Boot + Next.js](https://github.com/lionelchoque/spring-nextjs-auth) | JWT auth, Spring Security, SSR, production-ready Docker setup | Spring Boot, Next.js 14, PostgreSQL | ⏳ Planned |
| 06 | [TDD Showcase](https://github.com/lionelchoque/tdd-spring-showcase) | Test-driven development with Mockito + JUnit 5, JaCoCo coverage | Spring Boot, Mockito, JUnit 5 | ⏳ Planned |
| 07 | [MVC + Clean Architecture](https://github.com/lionelchoque/spring-mvc-clean-arch) | Layered architecture, global error handling, OpenAPI 3 docs | Spring Boot, Swagger, Bean Validation | ⏳ Planned |

---

## Currently building

- 🧱 Deepening **Java + Spring Boot** — JPA, Spring Security, TDD with Mockito
- 🤖 Studying **AI/ML architecture** — from ML fundamentals to agent-based systems
- 🏭 Consulting on **industrial data migration** — ETL pipelines for the water treatment sector
- 📝 Documenting everything on [LinkedIn](https://linkedin.com/in/YOUR_LINKEDIN)

---

## Stats

<p align="center">
  <img height="160" src="https://github-readme-stats.vercel.app/api?username=lionelchoque&show_icons=true&theme=default&hide_border=true&count_private=true&include_all_commits=true" />
  <img height="160" src="https://github-readme-stats.vercel.app/api/top-langs/?username=lionelchoque&layout=compact&theme=default&hide_border=true&langs_count=6" />
</p>

---

## Let's connect

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=flat&logo=linkedin&logoColor=white)](https://linkedin.com/in/YOUR_LINKEDIN)
[![Email](https://img.shields.io/badge/Email-EA4335?style=flat&logo=gmail&logoColor=white)](mailto:YOUR_EMAIL)

> *"I care about the why behind every technical decision — not just making things work, but understanding why they work that way."*
